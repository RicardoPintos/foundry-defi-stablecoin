 ...
 *
 * The system is designed to be as minimal as posible, and have the tokens mantain a 1 token == $1 peg.
 ...
 *
 * Our DSC system should always be "overcollateralized". At no point, should the value of all collateral be less or equal than the dollar backed value of all the DSC.
 *
...
contract DSCEngine is ReentrancyGuard {
    ...
    function burnDsc(uint256 amount) public moreThanZero(amount) {
        ...
        _revertIfHealthFactorIsBroken(msg.sender); // Back-up check
    }
     ...
     * @param user The user who has b        if (totalDscMinted == 0) return type(uint256).max;
     * roken the health factor. Their _healthFactor should be below MIN_HEALTH_FACTOR
     ...
     * @notice This function working assumes the protocol will be reoughly 200% overcollateralized in order for this to work
    ...
    function getCollateralTokensArray() external view returns (address[] memory) {
        return s_collateralTokens;
    }

    function getDscOwner() external view returns (address) {
        return i_dsc.owner();
    }
